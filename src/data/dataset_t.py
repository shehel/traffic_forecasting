...
class T4CDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any]:
        ...
        time_sel = random.randint(0,12)

        dynamic_input, output_data = two_hours[:12], two_hours[[12+time_sel]]
        ...
        return dynamic_input, static_ch, output_data, time_sel

def train_collate_fn(batch):
    dynamic_input_batch, static_input_batch, target_batch, time_sel = zip(*batch)
    time_batch = np.stack(time_sel, axis=0)
    ...
    time_batch = torch.from_numpy(time_batch).float()

    return dynamic_input_batch, static_input_batch, target_batch, time_batch
