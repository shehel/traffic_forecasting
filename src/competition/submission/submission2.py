            ...
            for competition_file in competition_files:
                ...
                with torch.no_grad():
                    for i in range(num_batches):
                        ...
                        map_name = competition_file.split('/')[3]+'_static.h5'
                        static = load_h5_file(competition_file.replace(competition_file.split('/')[4], map_name), sl=slice(0, 9), to_torch=False)
                        static = np.expand_dims(static, 0)
                        static = torch.from_numpy(static)
                        static = static.to(dtype=torch.float)
                        static = static.repeat(test_data.shape[0],1,1,1)
                        
                        if pre_transform is not None:
                            test_data: Union[torch.Tensor, torch_geometric.data.Data] = pre_transform(test_data, static=static, city=city, **additional_transform_args)
                        else:
...
def main(model_str: str, checkpoint: str, batch_size: int, device: str, data_raw_path: str, submission_output_dir: Optional[str] = None):
    ...
    #to_save = {"train_model": model}
    model = torch.load(checkpoint, map_location='cpu')
    #Checkpoint.load_objects(to_load=to_save, checkpoint=checkpoint_load)
